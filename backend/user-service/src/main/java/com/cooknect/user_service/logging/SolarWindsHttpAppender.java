package com.cooknect.user_service.logging;
...
public class SolarWindsHttpAppender extends AppenderBase<ILoggingEvent> {
    ...
    public void start() {
        ...
        if (this.url == null || this.url.isEmpty()) {
            addError("URL is not set for SolarWindsHttpAppender.");
            return;
        }
        // Crucial check: Ensure the token is provided.
        if (this.token == null || this.token.isEmpty() || this.token.contains("dummy-token")) {
            addError("Authorization token is not set or is a dummy token for SolarWindsHttpAppender. Appender will not start.");
            return;
        }
        ...
        this.httpClient = HttpClient.newBuilder()
                ...
                .build();
        super.start();
        addInfo("Solarwinds started for the URl: " + this.url);
    }
