...
const MEAL_STRUCTURE_DEFAULTS = {
  ...
  Dinner: {
    bgColor: "#f0f5ff",
    icon: (
      <Moon style={{ width: "20px", height: "20px", color: PRIMARY_COLOR }} />
    ),
    items: [],
  },
};
  ...
  return (
    ...
    >
      <div style={{ marginBottom: "16px" }}>
        ...
        >
          <Text style={{ color: fibreColor }}>
            Fibre: {localFibre.toFixed(1)}g
          </Text>
          <Text style={{ color: carbsColor }}>
            Carbs: {localCarbs.toFixed(1)}g
          </Text>
          <Text style={{ color: fatColor }}>Fat: {localFat.toFixed(1)}g</Text>
          <Text style={{ color: sugarColor }}>
            Sugar: {localSugar.toFixed(1)}g
          </Text>
        </div>
      </div>
...
const Nutrition = () => {
  ...
  const fetchAllRecipes = async (pageNumber = 1) => {
    try {
      const url = `http://localhost:8089/api/v1/recipes?page=${pageNumber}&size=3&sortBy=id&direction=asc`;
  ...
  const fetchMealData = async (mealType) => {
    try {
      const response = await fetch(`${API_BASE_URL}MealTypes/${mealType}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (!response.ok) {
        console.error(
          `Failed to fetch ${mealType} data: ${response.statusText}`
        );
        return [];
      }
  ...
  const fetchDailyMealSummaries = async () => {
    ...
    try {
      const fetchPromises = MEAL_TYPES_TO_FETCH.map((type) =>
        fetchMealData(type)
      );
      ...
      message.error(
        "Failed to load daily nutrition data. Check the backend connection."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleViewHistory = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}NutritionLogsByUserId`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      message.error(
        "Failed to load nutrition history. Please ensure the backend is running and accessible."
      );
    } finally {
  ...
  const historyColumns = [
    ...
    { title: "Meal Type", dataIndex: "mealType", key: "mealType" },
    {
      ...
      key: "fibre",
      render: (val) => (
        <Text style={{ color: fibreColor }}>{val.toFixed(1)}</Text>
      ),
    },
    {
      title: "Carbs (g)",
      ...
      key: "carbs",
      render: (val) => (
        <Text style={{ color: carbsColor }}>{val.toFixed(1)}</Text>
      ),
    },
    {
      title: "Fat (g)",
      ...
      key: "fat",
      render: (val) => (
        <Text style={{ color: fatColor }}>{val.toFixed(1)}</Text>
      ),
    },
    {
      title: "Sugar (g)",
      ...
      key: "sugar",
      render: (val) => (
        <Text style={{ color: sugarColor }}>{val.toFixed(1)}</Text>
      ),
    },
  ];
    ...
    >
      ...
      <Card
        ...
        title={
          ...
          >
            ...
            >
              <Dumbbell style={{ marginRight: "8px", color: PRIMARY_COLOR }} />{" "}
              Daily Macro Breakdown
            </Title>
            {/* Display Today's Date */}
            <Text strong style={{ color: PRIMARY_COLOR, fontSize: "16px" }}>
              {today}
            </Text>
          </div>
        }
      ...
      >
        <Row gutter={[24, 24]}>
          <Col xs={24} sm={12} lg={6}>
            <MacroGoalProgress
              label="Fibre"
              current={totals.fibre}
              goal={MOCK_GOALS.fibre}
              unit="g"
              color={fibreColor}
              loading={isLoading}
            />
      ...
      >
        ...
        >
          <Form.Item>
            <Text type="secondary" style={{ fontSize: "12px" }}>
              Select Recipe from the dropdown
            </Text>
            <Select
              showSearch
              ...
              onChange={(value) => {
                const selected = allRecipes.find(
                  (recipe) => recipe.id === value
                );
                setSelectedRecipe(selected);
                form.setFieldsValue({
                  recipeId: selected?.id,
                  recipeName: selected?.title,
                });
              }}
